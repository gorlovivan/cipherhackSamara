...
L.drawLocal = {
	draw: {
		...
		handlers: {
			...
			marker: {
				tooltip: {
					start: 'Нажмите на карту, чтобы поместить маркер'
				}
			},
			polygon: {
				tooltip: {
					...
					end: 'Нажмите первую точку, чтобы закрыть эту фигуру'
				}
			},
			polyline: {
				error: '<strong>Ошибка:</strong> Ломаная линия не может пересекаться!',
				tooltip: {
					start: 'Нажмите, чтобы начать рисовать линию',
					cont: 'Нажмите, чтобы продолжить рисование линии',
					end: 'Нажмите последнюю точку, чтобы закончить линию'
				}
			},
			rectangle: {
				tooltip: {
			...
			simpleshape: {
				tooltip: {
					end: 'Отпустите мышь, чтобы закончить рисунок'
				}
			}
		}
	},
	edit: {
		...
		handlers: {
			edit: {
				tooltip: {
					text: 'Перетащите вершины или метки для редактирования фигуры',
...
L.GeometryUtil = L.extend(L.GeometryUtil || {}, {
	...
	readableDistance: function (distance, isMetric) {
		...
		if (isMetric) {
			...
			if (distance > 1760) {
				distanceStr = (distance / 1760).toFixed(2) + ' миль';
			} else {
				distanceStr = Math.ceil(distance) + ' ярдов';
			}
		}
...
L.Polyline.include({
	...
	spliceLatLngs: function (index, count, toAdd) {
